   ...
   AND tbl_questions.exam_id = '" . $exam_id . "'";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_object()) {
            ...
            $_answer = $row->answer;
            $sql = "INSERT INTO tbl_question_answer (`question_id`,`exam_id`,`student_id`,`answer`, `correct_answer`,`type`)
            VALUES ('$question_id', '$exam_id','$student_id','" . strtoupper($_POST['an' . $starting_mark . '']) . "', '" . strtoupper(base64_decode($_POST['ran' . $starting_mark . ''])) . "','practice')";
...
$sql = "UPDATE tbl_assessment_practice_records SET score='$percent_score', status='$status' WHERE record_id='$record'";

if ($conn->query($sql) === TRUE) {


    header("location:../practice_assessment-info.php?exam_id=$exam_id");
} else {
    header("location:../practice_assessment-info.php?exam_id=$exam_id");
}
