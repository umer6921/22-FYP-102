...
if (isset($_SESSION['record_id'])) {
    ...
    $sql = "SELECT * FROM tbl_assessment_practice_records WHERE record_id = '$record_id'";
            ...
            <div id="main-wrapper">
                ...
                <div class="row">
                    <div class="col">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h3 class="panel-title">Assessment Answer Sheet</h3>
                            </div>
                            <div class="panel-body">
                                <div class="row">
                                    <div class="col">
                                        <table class="table table-strip">
                                            ...
                                            <tbody>
                                                ...
                                                $sql = "SELECT tbl_examinations.exam_name,tbl_examinations.passmark,
                                                tbl_examinations.re_exam,tbl_questions.question,tbl_question_answer.answer,
                                                tbl_question_answer.correct_answer
                                                FROM tbl_question_answer
                                                INNER JOIN tbl_questions AS tbl_questions ON tbl_questions.question_id=tbl_question_answer.question_id
                                                INNER JOIN tbl_examinations AS tbl_examinations ON tbl_examinations.exam_id=tbl_question_answer.exam_id
                                                INNER JOIN tbl_assessment_practice_records AS tbl_assessment_practice_records ON tbl_assessment_practice_records.exam_id=tbl_examinations.exam_id
                                                WHERE tbl_question_answer.student_id='{$myid}' AND tbl_examinations.exam_id='{$exam_id}'";
                                                $result = $conn->query($sql);
                                                if ($result->num_rows > 0) {
                                                    $serail_no = 1;
                                                    while ($row = $result->fetch_assoc()) { ?>
                                                        <tr>
                                                            <td class="text-left"><?= $serail_no++ ?></td>
                                                            <td class="text-left"><?= $row['exam_name'] ?></td>
                                                            <td class="text-left"><?= $row['question'] ?></td>
                                                            <td class="text-left"><?= $row['answer'] ?></td>
                                                            <td class="text-left"><?= $row['correct_answer'] ?></td>
                                                        </tr>
                                                <?php }
                                                }
                                                $conn->close();
