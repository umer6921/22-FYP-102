...
if (isset($_SESSION['current_examid'])) {
    ...
    if ($retake_status == "1") {
        $sql = "DELETE FROM tbl_assessment_practice_records WHERE student_id = '$myid' AND exam_id = '$exam_id'";
...
$sql = "SELECT * FROM tbl_assessment_practice_records WHERE student_id = '$myid' AND tbl_assessment_practice_records.exam_id='{$exam_id}'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {

    while ($row = $result->fetch_assoc()) {
        header("location:./practice-take-assessment.php.php?id=$exam_id");
    }
} else {
    $myname = "$myfname $mylname";
    $recid = 'RS' . get_rand_numbers(14) . '';

    $sql = "INSERT INTO tbl_assessment_practice_records (record_id, student_id, student_name, exam_name, exam_id, score, status, next_retake, date)
                    ...
                    <div class="panel panel-white">
                        <div class="panel-body">
                            <div class="tabs-below" role="tabpanel">
                                <form action="pages/submit_practice_assessment.php" method="POST" name="quiz" id="quiz_form">
                                    <div class="tab-content">
                                        <?php
                                        include '../database/config.php';
                                        $sql = "SELECT *
                                        FROM tbl_questions q WHERE exam_id = '{$exam_id}' ORDER BY RAND()";
                                        $result = $conn->query($sql);
                                        ...
                                        if ($result->num_rows > 0) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                $qs = $row['question'];
                                                $type = $row['type'];
                                                $optionArr = array("option1" => $row['option1'], "option2" => $row['option2'], "option3" => $row['option3'], "option4" => $row['option4']);
                                                shuffle($optionArr);
                                                $op1 = $optionArr[0];
                                                $op2 = $optionArr[1];
                                                $op3 = $optionArr[2];
											...
											<div role="tabpanel" class="tab-pane active fade in" id="tab' . $qno . '">
                                             <p><b class="text-question_no">' . $qno . '.</b> ' . $qs . '</p>
                                    ...
                                    <ul class="nav nav-tabs" role="tablist">
                                        ...
                                        <input type="hidden" name="student_id" value="<?php echo "$myid"; ?>">
