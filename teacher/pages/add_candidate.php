...
$sql = "SELECT * FROM tbl_users WHERE email = '$email' OR phone = '$phone' tbl_users.role='student'";
...
if ($result->num_rows > 0) {
	...
	$sph = $row['phone'];
	if ($sem == $email) {
	 header("location:../candidate.php?rp=1189");	
	}else{
	
	if ($sph == $phone) {
	 header("location:../candidate.php?rp=2074");	
	}else{
...
if ($conn->query($sql) === TRUE) {
  header("location:../candidate.php?rp=6310");
} else {
  header("location:../candidate.php?rp=9157");
}
